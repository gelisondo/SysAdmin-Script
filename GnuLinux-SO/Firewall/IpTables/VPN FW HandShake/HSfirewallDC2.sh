...
$IPTABLES -Z
	...
	#CREO QUE ESTO ES AL RE PEDO
	#POR QUE EL -F -X ABRIA LIMPIADO TODO, PERO DEVEMOS PROVARLO
...
## RANGOS PARA LA VPN.
#$IPTABLES -A INPUT -i $IF_EXT -s 10.0.0.0/8 -m limit --limit 5/minute -j LOG  --log-prefix "A reserved DROP: "
...
# la nuestra
## Clase A
#$IPTABLES -A INPUT -i $IF_EXT -d 10.0.0.0/8 -m limit --limit 5/minute -j LOG  --log-prefix "A reserved DROP: "
#$IPTABLES -A INPUT -i $IF_EXT -d 10.0.0.0/8 -j DROP
...
#permitimos el accesso a ssh
# si le ponemos -s ip datacenter y pasa por otro router no anda. bueno tiene pinta 
# AVERIGUAR, O DIEGO DEVE SAVER!!!!!!!!!!!!!!!!!!!!!!!
#$IPTABLES -A INPUT -p tcp -s 0/0 -m tcp --dport 22 --sport 1024:65535 -m state --state NEW -j LOG --log-prefix "[FW -SSH]"
#$IPTABLES -A INPUT -p tcp -s 0/0 -m tcp --dport 22 --sport 1024:65535  -j ACCEPT
#$IPTABLES -A OUTPUT -p tcp --sport 22 -m state --state RELATED,ESTABLISHED -j ACCEPT
### Probado ok!!!!

#ORTAMOS EL ACCESO POR SSH AL SERVER VPN DES
...
$IPTABLES  -t nat -A PREROUTING -i $IF_EXT -p tcp  --dport 445 --sport 1024:65535 -j DNAT --to $IP_WEB:445
...
###########################################################################################################################
#Ya provado y funciona!!!! hay comunicacion entre los quipos atraves
#de las vpns
...
##########################################################################################################################
	...
	$IPTABLES -A INPUT -i $IF_TUN -p tcp --sport 1024:65535 --dport 22 -m state --state NEW -j LOG --log-prefix "[Intento de conexion SSH -sourse tun]"	
	...
	###
        ...
        # Ver bien y Provar cuando este el server VOIP!!!!!!!!
	### Servicios VoIP utilizando 

   $IPTABLES -A FORWARD -i $IF_TUN -o $IF_LAN -p tcp  --dport 4569 --sport 1024:65535 -m state --state NEW -j LOG --log-prefix "[S Tun - IAX2/tcp]"   
	$IPTABLES -A FORWARD -i $IF_TUN -o $IF_LAN -p udp  --dport 4569 --sport 1024:65535 -m state --state NEW -j LOG --log-prefix "[S Tun - IAX2/udp]"
	# CONEXION IAX2 TCP
	...
	# CONEXION IAX2 UDP
	$IPTABLES -A FORWARD -i $IF_TUN -o $IF_LAN -p udp  --dport 4569 --sport 1024:65535 -j ACCEPT
	$IPTABLES -A FORWARD -i $IF_LAN -o $IF_TUN -p udp  --sport 4569  --dport 1024:65535 -m state --state RELATED,ESTABLISHED -j ACCEPT
...
$IPTABLES -A FORWARD -s $RED_LAN -i $IF_LAN -p tcp --dport 443 -j ACCEPTS
